...
### Design stance
The main page is no longer only a trip landing page. It is the current user’s dashboard: what needs their attention, what they owe or are owed, and the fastest path into the current/future trips or the full trip archive.
...
- **Removed the oversized dashboard intro:** the page now starts directly with the current/future trip card to preserve space.
...
- **Moved attention above money:** actionable items now appear before the balance summary so decisions/follow-ups lead the user dashboard.
- **Renamed future-trip language:** “Future trips” is used where the UI refers to upcoming travel beyond the current trip.
- **Removed current-trip buttons:** the featured trip card is now a single tappable surface to save vertical space.
...
- **Kept the large current/future trip card:** the current trip still visually anchors the page, but its surfaced detail is about what the user needs to do next.
...
1. **Featured current/future trip card** — still prominent and emotional, but more compact; the whole card opens the current trip.
2. **Your money across trips** — net outstanding, owed-to-you, and you-owe rows.
3. **User metrics** — decisions, invites, and future trips.
4. **Needs your attention** — actionable cross-trip tasks.
5. **Current & future trips list** — direct entry points into active and upcoming travel.
...
- Should “Current” in the bottom nav mean the current trip only, or current/future trip depending on trip state?
